...
// Captures conversation content from claude.ai using a Debounced MutationObserver
...
(function () {
  ...
  async function init() {
    ...
    if (!state.activeProject || !state.captureEnabled || state.isBlocked) {
      console.log('[Continuum] Claude capture inactive');
      return;
    }

    console.log('[Continuum] Claude capture active for project:', state.activeProject.name);
    startObserving();
  ...
  function startObserving() {
    ...
    observer = new MutationObserver((mutations) => {
      ...
      for (const mutation of mutations) {
        // Ignore mutations happening inside Claude's input field
        if (mutation.target.isContentEditable || mutation.target.classList?.contains('ProseMirror')) {
          continue;
        }
        shouldDebounce = true;
      }
  ...
  function flushBuffer() {
    ...
    const processElement = (msg) => {
      ...
      // Determine role based on common Claude DOM attributes
      const isUser = msg.getAttribute('data-testid')?.includes('user') || msg.innerHTML.includes('User');
